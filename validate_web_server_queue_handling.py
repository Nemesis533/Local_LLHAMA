...
def check_web_server_error_handling():
    ...
    for name, pattern in anti_patterns.items():
        ...
        if name == "bare_put_no_timeout":
            ...
            if puts_without_timeout:
                print(f"⚠️  {name}: {len(puts_without_timeout)} instance(s) - {puts_without_timeout}")
    ...
    requirements = {
        "Queue validation": results["queue_validation_action"] >= 1 and results["queue_validation_web"] >= 1,
