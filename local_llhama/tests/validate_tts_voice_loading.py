...
def check_tts_voice_loading():
    ...
    if tts_section:
        ...
        loglevel_count = len(re.findall(r'LogLevel\.(INFO|WARNING|CRITICAL)', tts_content))
    else:
        loglevel_count = 0
    
    # Count error handling
    tts_try_blocks = len(re.findall(r'\btry:', tts_content)) if tts_section else 0
    tts_except_blocks = len(re.findall(r'\bexcept\b', tts_content)) if tts_section else 0
