...
from .routes.chat_history_routes import init_chat_history_routes, chat_history_bp
...
class LocalLLHAMA_WebService:
    def __init__(self, host='0.0.0.0', port=5001, action_message_queue=None, web_server_message_queue=None, chat_message_queue=None, pg_client=None):
        ...
        try:
            ...
            print(f"[WebServer] [{LogLevel.INFO.name}] PostgreSQL client created for web service process")
        except Exception as e:
            print(f"[WebServer] [{LogLevel.WARNING.name}] Failed to create PostgreSQL client: {repr(e)}")
            self.pg_client = None
        ...
        try:
            ...
            print(f"[WebServer] [{LogLevel.WARNING.name}] Failed to initialize conversation loader: {repr(e)}")
        ...
        self.app.register_blueprint(chat_history_bp)  # Register chat_history_bp before init
    ...
    def _setup_authentication(self):
        ...
        try:
            ...
            print(f"{self.class_prefix_message} [{LogLevel.INFO.name}] Authentication manager initialized")
        except Exception as e:
            print(f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Failed to initialize auth manager: {repr(e)}")
            raise
    ...
    def set_system_controller(self, system_controller):
        ...
        if self.conversation_loader and hasattr(system_controller, 'command_llm'):
            try:
                system_controller.command_llm.conversation_loader = self.conversation_loader
                print(f"{self.class_prefix_message} [{LogLevel.INFO.name}] Conversation loader attached to OllamaClient")
            except Exception as e:
                print(f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Failed to attach conversation loader: {repr(e)}")
    ...
    def send_chat_message(self, text: str, client_id: str = None, conversation_id: str = None):
        """
        ...
        """
        print(f"{self.class_prefix_message} [{LogLevel.INFO.name}] Received Chat from User: {text} (client={client_id}, conversation={conversation_id})")
