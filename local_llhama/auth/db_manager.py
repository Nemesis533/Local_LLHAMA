...
class User:
    """
    @brief Simple User class to manage user attributes and authentication.
    """
...
class DatabaseManager:
    ...
    def get_user_by_username(self, username):
        ...
        results = self.pg_client.execute_query_dict(
            ...
            "is_admin, can_access_chat, must_change_password "
    ...
    def update_password(self, username, new_password):
        ...
        if len(new_password) < 8:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Password must be at least 8 characters"
            )
            return False
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  Password updated successfully for user: {username}"
            )
            return True
        except Exception as e:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to update password for user: {username} - {e}"
            )
            return False
    ...
    def check_password_reset_needed(self):
        ...
        if reset_password and len(reset_password) >= 8:
            ...
            if success:
                print(
                    f"{self.class_prefix_message} {LogLevel.INFO}  Admin password reset from .env file"
                )
                print(
                    f"{self.class_prefix_message} {LogLevel.WARNING}  Remove RESET_ADMIN_PASSWORD from .env for security!"
                )
                return True
    ...
    def get_all_users(self):
        """
        Retrieve all users from database including their attributes.
            ...
            for row in results:
                users.append(
                    User(
                        ...
                        created_at=str(row["created_at"]),
                        last_login=(
                            str(row["last_login"]) if row["last_login"] else None
                        ),
                        is_active=bool(row["is_active"]),
                        is_admin=bool(row["is_admin"]),
                        can_access_chat=bool(row["can_access_chat"]),
    ...
    def create_user(
        ...
        try:
            self.pg_client.execute_write(
                ...
                "can_access_chat, must_change_password) VALUES (%s, %s, %s, %s, %s)",
                (
                    username,
                    password_hash,
                    is_admin,
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  User created successfully: {username}"
            )
            return True
        except Exception as e:
            error_msg = str(e)
            if "unique" in error_msg.lower() or "duplicate" in error_msg.lower():
                print(
                    f"{self.class_prefix_message} {LogLevel.WARNING}  Username already exists: {username}"
                )
            else:
                print(
                    f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to create user: {e}"
                )
            return False
    ...
    def update_user_permissions(
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  Permissions updated for user: {username}"
            )
            return True
        except Exception as e:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to update permissions: {e}"
            )
            return False
    ...
    def reset_user_password(self, username, new_password, must_change=True):
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  Password reset for user: {username}"
            )
            return True
        except Exception as e:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to reset password: {e}"
            )
            return False
    ...
    def clear_password_change_flag(self, username):
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to clear password flag: {e}"
            )
            return False

    def delete_user(self, username):
        """
        Delete a user and all their conversations (cannot delete admin).
        ...
        if username == "admin":
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Cannot delete admin user"
            )
            return False
        ...
        try:
            ...
            self.pg_client.execute_write(
                "DELETE FROM conversations WHERE username = %s", (username,)
            )
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  Deleted conversations for user: {username}"
            )
            ...
            print(
                f"{self.class_prefix_message} {LogLevel.INFO}  User deleted: {username}"
            )
            return True
        except Exception as e:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING}  Failed to delete user: {e}"
            )
            return False
