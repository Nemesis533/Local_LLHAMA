...
class AuthManager:
    ...
    def verify_credentials(self, username, password):
        ...
        if not username or not password:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING} Empty username or password provided"
            )
            return None
        ...
        if not user:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING} Login attempt for non-existent user: {username}"
            )
            return None

        if not user.is_active:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING} Login attempt for inactive user: {username}"
            )
            return None
        ...
        try:
            ...
            if bcrypt.checkpw(
                ...
                print(
                    f"{self.class_prefix_message} {LogLevel.INFO} Successful login for user: {username}"
                )
                ...
                print(
                    f"{self.class_prefix_message} {LogLevel.WARNING} Failed login attempt for user: {username}"
                )
                return None
        except Exception as e:
            print(
                f"{self.class_prefix_message} {LogLevel.WARNING} Password verification error for user {username}: {str(e)}"
            )
            return None
