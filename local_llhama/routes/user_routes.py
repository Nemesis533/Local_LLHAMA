...
def from_user_text():
    ...
    conversation_id = data.get(
        "conversation_id"
    )  # Get conversation_id from frontend (may be None for new)
    ...
    with current_app.app_context():
        ...
        returned_conversation_id = service.send_chat_message(
            text=user_text, client_id=client_id, conversation_id=conversation_id
        )

    # Return success + conversation_id for frontend to store
    return jsonify({"success": True, "conversation_id": returned_conversation_id})
