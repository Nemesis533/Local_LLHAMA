...
class StateHandlers:
    ...
    def handle_recording(self):
        ...
        try:
            ...
            if transcription_words > 4:
                ...
                print(f"{self.log_prefix} [{LogLevel.INFO.name}] Transcription only had {transcription_words} words, returning to listening")
                self.sm.state_manager.transition(self.sm.State.LISTENING)
        except Exception as e:
            print(f"{self.log_prefix} [{LogLevel.CRITICAL.name}] Recording failed: {type(e).__name__}: {e}")
            self.sm.state_manager.transition(self.sm.State.LISTENING)
    ...
    def handle_send_commands(self):
        ...
        try:
            ...
            print(f"{self.log_prefix} [{LogLevel.WARNING.name}] Command queue empty when expected, returning to LISTENING")
            self.sm.state_manager.transition(self.sm.State.LISTENING)
        except Exception as e:
            print(f"{self.log_prefix} [{LogLevel.CRITICAL.name}] Failed to send command: {type(e).__name__}: {e}")
            self.sm.state_manager.transition(self.sm.State.LISTENING)
    ...
    def handle_speaking(self):
        ...
        if transcription:
            try:
                ...
                print(f"{self.log_prefix} [{LogLevel.CRITICAL.name}] Speaking failed: {type(e).__name__}: {e}")
    ...
    def _command_worker(self):
        ...
        if structured_output:
            if structured_output.get("commands"):
                print(f"{self.log_prefix} [{LogLevel.INFO.name}] Structured Commands: {structured_output}")
                success = self.sm.queue_manager.put_safe(
                ...
                if success:
                    print(f"{self.log_prefix} [{LogLevel.INFO.name}] Successfully put command into queue")
