...
class SoundPlayer:
    ...
    def __init__(self, base_path):
        ...
        for attempt in range(3):
            try:
                ...
                print(
                    f"{self.class_prefix_message} [{LogLevel.INFO.name}] Pygame mixer initialized successfully"
                )
                break
            except pygame.error as e:
                print(
                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Failed to init mixer (attempt {attempt + 1}/3): {e}"
                )
                time.sleep(0.5)
            except Exception as e:
                print(
                    f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Unexpected error initializing mixer: {type(e).__name__}: {e}"
                )
                break

        if not mixer_initialized:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Failed to initialize pygame mixer after 3 attempts - sounds will be disabled"
            )
        ...
        self.sounds_root_folder = (
            f"{self.base_path}/sounds/"  # Base folder for sound files
        )
    ...
    def load_sound(self, sound_name):
        ...
        if sound_name not in self.loaded_sounds:
            audio_path = (
                f"{self.sounds_root_folder}{self.sounds_dictionary[sound_name]}"
            )
            try:
    ...
    def play(self, sound_to_play, volume: float = 1.0, wait_for_finish=True):
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Playback error: {e}"
            )
        except Exception as e:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Unexpected playback error: {type(e).__name__}: {e}"
            )
...
class TextToSpeech:
    ...
    def __init__(self, voice_dir: str, language_models: dict = None):
        ...
        if language_models:
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Using custom language models: {list(language_models.keys())}"
            )
        else:
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.WARNING.name}] No language models provided, using defaults"
            )

        # Validate voice directory
        if not self.voice_dir.exists():
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice directory not found: {voice_dir}"
            )
            raise FileNotFoundError(f"Voice directory not found: {voice_dir}")

        if not self.voice_dir.is_dir():
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice path is not a directory: {voice_dir}"
            )
        ...
        try:
            ...
            if not voice_files:
                print(
                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] No .onnx voice files found in {voice_dir}"
                )
            else:
                print(
                    f"{self.class_prefix_message} [{LogLevel.INFO.name}] Found {len(voice_files)} voice file(s) in {voice_dir}"
                )
        except Exception as e:
            print(
        ...
        for init_attempt in range(3):
            try:
                if init_attempt > 0:
                    print(
                        f"{self.class_prefix_message} [{LogLevel.INFO.name}] PyAudio init attempt {init_attempt + 1}/3..."
                    )
                ...
                print(
                    f"{self.class_prefix_message} [{LogLevel.INFO.name}] PyAudio initialized successfully ({device_count} devices found)"
                )
                ...
                if device_count > 0:
                    ...
                    print(
                        f"{self.class_prefix_message} [{LogLevel.WARNING.name}] PyAudio found 0 devices, retrying..."
                    )
                    self.p.terminate()
                    self.p = None
            except Exception as init_err:
                print(
                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] PyAudio init attempt {init_attempt + 1} failed: {init_err}"
                )
                if self.p:
                    try:
                        self.p.terminate()
        ...
        try:
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Testing output device {self.output_device_index}..."
            )
            ...
            test_stream.close()
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Output device test successful"
            )
        except (OSError, IOError) as test_err:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Output device test failed: {test_err}"
            )
            ...
            for i in range(device_count):
                ...
                try:
                    ...
                    if dev_info["maxOutputChannels"] > 0:
                        ...
                        self.output_device_index = i
                        print(
                            f"{self.class_prefix_message} [{LogLevel.INFO.name}] Found working alternative device: {dev_info['name']} (index: {i})"
                        )
                        break
                except:
                    continue
            ...
            if self.output_device_index is None:
                raise RuntimeError(
                    f"All output devices failed to open. Original error: {test_err}"
                )
    ...
    def preprocess_text(self, text: str) -> str:
        """
        @brief Preprocess text for better TTS pronunciation.
        """
        text = text.strip()
    ...
    def select_voice_by_lang(self, lang_tag: str):
        """
        @brief Select and load a voice for the specified language with caching and error handling.
        """
        ...
        if lang_tag in self.voice_cache:
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Using cached voice for language: {lang_tag}"
            )
            return

        # Validate language tag
        if lang_tag not in self.language_models:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Unsupported language tag: {lang_tag}"
            )
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Supported languages: {list(self.language_models.keys())}"
            )
            raise ValueError(
                f"Unsupported language tag: {lang_tag}. Supported: {list(self.language_models.keys())}"
            )

        # Get model filename from configuration
        model_filename = self.language_models[lang_tag]
        voice_file = self.voice_dir / model_filename

        # Check if voice file exists
        if not voice_file.exists():
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice file not found: {voice_file}"
            )
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Expected model: {model_filename} for language '{lang_tag}'"
            )
            ...
            try:
                ...
                if all_voices:
                    print(
                        f"{self.class_prefix_message} [{LogLevel.INFO.name}] Available voices: {[v.name for v in all_voices]}"
                    )
                else:
                    print(
                        f"{self.class_prefix_message} [{LogLevel.WARNING.name}] No .onnx files found in {self.voice_dir}"
                    )
            except Exception:
                pass
            raise FileNotFoundError(
                f"Voice model '{model_filename}' not found for language '{lang_tag}' in {self.voice_dir}"
            )

        # Validate voice file
        if not voice_file.exists():
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice file does not exist: {voice_file}"
            )
            raise FileNotFoundError(f"Voice file not found: {voice_file}")

        if not voice_file.is_file():
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice path is not a file: {voice_file}"
            )
        ...
        try:
            ...
            if file_size == 0:
                print(
                    f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice file is empty: {voice_file}"
                )
                raise ValueError(f"Voice file is empty: {voice_file}")
            if file_size < 1000:  # Less than 1KB is suspicious
                print(
                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Voice file suspiciously small ({file_size} bytes): {voice_file}"
                )
        except Exception as e:
            print(
                f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Could not check voice file size: {e}"
            )
        ...
        try:
            ...
            self.current_lang = lang_tag
            print(
                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Successfully loaded voice: {voice_file.name}"
            )
        except FileNotFoundError as e:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice file not found during load: {e}"
            )
            raise
        except PermissionError as e:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Permission denied reading voice file: {e}"
            )
            raise RuntimeError(
                f"Cannot read voice file (permission denied): {voice_file}"
            )
        except Exception as e:
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Failed to load voice file: {type(e).__name__}: {e}"
            )
    ...
    def speak(self, text: str, lang_tag: str):
        ...
        if not text:
            raise ValueError(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Empty or invalid text"
            )
        ...
        try:
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Voice selection failed: {e}"
            )
            return
        ...
        try:
                ...
                while retry_count <= max_retries:
                    try:
                        if stream is None:
                            ...
                            print(
                                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Opening audio stream: device={self.output_device_index}, rate={chunk.sample_rate}, channels={chunk.sample_channels}"
                            )
                            ...
                            print(
                                f"{self.class_prefix_message} [{LogLevel.INFO.name}] Audio stream opened successfully"
                            )
                        ...
                        retry_count += 1
                        print(
                            f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Audio stream error (attempt {retry_count}/{max_retries + 1}): {e}"
                        )
                        print(
                            f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Device index: {self.output_device_index}, PyAudio valid: {self.p is not None}"
                        )
                        ...
                        if self.p is not None:
                            try:
                                device_count = self.p.get_device_count()
                                print(
                                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] PyAudio currently sees {device_count} devices"
                                )
                                if self.output_device_index < device_count:
                                    ...
                                    print(
                                        f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Device {self.output_device_index} info: {dev_info['name']}, outputs: {dev_info['maxOutputChannels']}"
                                    )
                            except Exception as diag_err:
                                print(
                                    f"{self.class_prefix_message} [{LogLevel.WARNING.name}] Diagnostic check failed: {diag_err}"
                                )
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Audio device error during speech: {e}"
            )
            if stream:
                try:
                    stream.close()
            ...
            print(
                f"{self.class_prefix_message} [{LogLevel.CRITICAL.name}] Unexpected error during speech: {type(e).__name__}: {e}"
            )
            if stream:
                try:
                    stream.close()
