        ...
        if actions:
            if entity_id == "home_weather":
                functions_desc.append(
                    f"- {entity_id}: Get weather forecast for your home location"
                )
                functions_desc.append(
                    f'  Example: {{"action": "home_weather", "target": "home_weather"}}'
                )
            elif entity_id == "web_search":
                functions_desc.append(
                    f"- {entity_id}: Search allowed websites for information"
                )
                functions_desc.append(
                    f'  Example: {{"action": "web_search", "target": "web_search", "data": {{"query": "your search query"}}}}'
                )
                functions_desc.append(
                    f'  Optional: Add "website" parameter to search specific site'
                )
            else:
                functions_desc.append(
                    f"- {entity_id}: Available actions: {', '.join(actions)}"
                )
                functions_desc.append(
