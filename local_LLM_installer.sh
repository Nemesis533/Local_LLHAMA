...
if [[ "$INSTALL_TORCH" == "yes" ]]; then
    ...
    read -p "Install CPU-only PyTorch or CUDA-enabled? (cpu/cuda) [cuda]: " -r TORCH_FLAVOR
    TORCH_FLAVOR=${TORCH_FLAVOR:-cuda}
    if [[ "$TORCH_FLAVOR" == "cpu" ]]; then
        ...
        pip install --index-url https://download.pytorch.org/whl/cpu torch==2.1.2 || {
            echo "WARNING: Automatic CPU PyTorch install failed. Please follow https://pytorch.org/get-started/locally/ to install the correct wheel for your platform."
        }
    else
        echo "Installing CUDA-enabled PyTorch 2.1.2 with cu121 wheels (ensure CUDA toolkit is installed)..."
        pip install torch==2.1.2 torchvision==0.16.2 torchaudio==2.1.2 --index-url https://download.pytorch.org/whl/cu121 || {
            echo "WARNING: Automatic CUDA PyTorch install failed. Please follow https://pytorch.org/get-started/locally/ to install the correct CUDA wheel for your GPU/CUDA version."
        }
    fi
fi
